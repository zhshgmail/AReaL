...
"""
        ...
        # Mock name_resolve and stats_logger to avoid dependencies
        with patch("areal.api.cli_args.name_resolve"), patch(
            "areal.api.cli_args.StatsLogger"
        ), patch.dict("os.environ", {"RANK": "1"}):
            cfg, _ = load_expr_config([str(config_file)], GRPOConfig)
...
"""
        ...
        with patch("areal.api.cli_args.name_resolve"), patch(
            ...
            cfg, _ = load_expr_config(
                [str(config_file), "enable_segment_wise_ppo=false"], GRPOConfig
            )
...
class TestWorkflowExecutorFactory:
    ...
    @patch("areal.api.workflow_factory.WorkflowExecutor")
...
"""
        ...
        with patch("areal.api.cli_args.name_resolve"), patch(
            ...
            cfg1, _ = load_expr_config([str(config_file)], GRPOConfig)

        with patch("areal.api.cli_args.name_resolve"), patch(
            "areal.api.cli_args.StatsLogger"
        ), patch.dict("os.environ", {"RANK": "1"}):
            cfg2, _ = load_expr_config([str(config_file)], GRPOConfig)
