...
class ProximalRecomputer:
    ...
    def recompute_for_sample(self, trajectory: dict, prompt_len: int) -> None:
        ...
        output_version = trajectory.get("output_version", [])

        if not output_version:
            return

        # Check if this is a v-1 sample (exactly current_ver - 1)
        # For unit testing, we use a simplified check on all output versions
        max_ver = max(output_version) if output_version else -1
        if max_ver != current_ver - 1:
            return  # Not v-1, skip

        # Recompute using the inference engine
        input_ids = trajectory["input_ids"]
        new_logprobs = self.inference_engine.recompute_output_logprobs_sync(
            input_ids=input_ids,
            start_index=prompt_len,
        )

        # Update the trajectory dict
        trajectory["proximal_logprobs_t"] = new_logprobs
    ...
    def recompute_batch(self, batch: List[dict], prompt_len: int) -> None:
        ...
        for trajectory in batch:
            self.recompute_for_sample(trajectory, prompt_len)
    ...
    def _recompute_queue_proximal_t(
        self, output_queue: "RolloutQueue", current_ver: int
    ) -> int:
