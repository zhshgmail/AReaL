...
def create_staleness_strategy(
    ...
    if config.enable_segment_wise_ppo:
        return SegmentWisePPOStrategy(config, logger=logger)
    else:
...
def create_filtered_capacity_modifier(
    ...
    """
    if config.enable_segment_wise_ppo:
        return FilteredSamplesCapacityModifier(logger=logger)
    return None
...
def create_workflow_executor(
    ...
    staleness_strategy = create_staleness_strategy(config, logger)
    ...
    filtered_capacity_modifier = create_filtered_capacity_modifier(config, logger)
    ...
    # Initialize the executor (starts AsyncTaskRunner and creates staleness_manager if None)
