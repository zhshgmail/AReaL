...
class SegmentWisePPOStrategy(StalenessControlStrategy):
    ...
    def filter_stale_from_cache(
        ...
        for td in result_cache:
            if self.is_sample_too_stale(td, current_ver, config):
                dropped_cache += 1
            else:
                filtered.append(td)

        result_cache.clear()
        result_cache.extend(filtered)

        if dropped_cache:
            logger.info(f"[CacheFilter] dropped_cache={dropped_cache} size={len(result_cache)}")
