import pytest
from sympy import Float, Max, Rational, Symbol, exp, nsimplify, pi, sqrt
...
def test_max_float():
    ...
    assert_equal(
        "\\max(1234.56789, 1234.5678901)",
        Max(Rational("1234.56789"), Rational("1234.5678901")),
    )
    ...
    assert_equal(
        "\\max(-21E-12, 0.00005)",
        Max(nsimplify(Rational("-21E-12")), Rational("0.00005")),
        symbolically=True,
    )
...
def test_max_fraction():
    ...
    assert_equal(
        "\\max(-4/3, -2/1, 0/9, -3)",
        Max(Rational("-4/3"), Rational("-2/1"), Rational("0/9"), -3),
    )
...
def test_max_expr():
    ...
    assert_equal(
        "\\sqrt{\\max(99.9999999999999, 100)}",
        sqrt(Max(Rational("99.9999999999999"), 100)),
    )
    assert_equal(
        "\\max(274/(5+2), \\exp(12.4), 1.4E2)",
        Max(Rational(274, 5 + 2), exp(Rational("12.4")), Rational("1.4E2")),
    )
...
def test_max_multiarg():
    ...
    assert_equal(
        "\\max(5.8,7.4, 2.2,-10)",
        Max(Rational("5.8"), Rational("7.4"), Rational("2.2"), -10),
    )
    assert_equal(
        "\\max(\\pi,12E2,84,\\sqrt{5},12/5)",
        Max(pi, Rational("12E2"), 84, sqrt(5), Rational("12/5")),
    )
