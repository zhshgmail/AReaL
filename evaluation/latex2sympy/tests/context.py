...
from sympy import Add, Mul, Pow, Rational, Symbol, pi, simplify, sqrt, srepr
...
def get_simple_examples(func):
    """Returns an array of tuples, containing the string `input`, sympy
    `output` using the provided sympy `func`, and `symbolically` boolean for
    calling `compare`."""
    return [
        ("1.1", func(1.1), False),
        ("6.9", func(6.9), False),
    ...
    if symbolically:
        ...
        try:
            ...
            if (
                ...
                or expected.is_number
            ):
                assert (
                    actual == expected
                    or actual - expected == 0
                    or simplify(actual - expected) == 0
                )
            else:
