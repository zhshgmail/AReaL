...
def get_metrics(fname_pattern, tokenizer, is_greedy, data_name=None):

    generated = []
    lengths = []
    results = {}
    idx_to_length = defaultdict(list)

    for fname in sorted(glob(fname_pattern)):
        datas = load_jsonl(fname)
        for data in tqdm(datas, desc=fname):

            # tokenize
            generated.extend(data["code"])
            idx_to_length[data['idx']].extend(tokenizer(data["code"], return_length=True)['length'])

            # if 'generated' list is too long, more than 2000 elements, then
            # tokenize and get the length. Finally, reset the list to empty.
    ...
    output_path = f"/home/bruceli/projects/results/sample_lengths/maxstale0_{data_name}_sample_length_greedy={is_greedy}.txt"
    ...
    # with open(f"/home/bruceli/projects/results/sample_lengths/maxstale0_{data_name}_idx_sample_length_greedy={is_greedy}_dict.json", "w") as f:
    #     json.dump(idx_to_length, f, indent=4)
    # print(f"Saved {len(idx_to_length)} lengths to /home/bruceli/projects/results/sample_lengths/maxstale0_{data_name}_idx_sample_length_greedy={is_greedy}_dict.json")
...
def process_single_data_name(args, data_name, base_dir, tokenizer):
    ...
    greedy_length_metrics, _ = get_metrics(
        os.path.join(cur_dir, greedy_prefix + ".jsonl"), tokenizer, True, data_name
    )
    sampling_metrics, sampling_results = get_metrics(
        os.path.join(cur_dir, sampling_prefix + ".jsonl"), tokenizer, False, data_name
    )
...
if __name__ == "__main__":
    ...
    if True or not os.path.exists(result_path) or args.overwrite or args.evaluate_train:
